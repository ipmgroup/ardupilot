...
void RangeFinder::detect_instance(uint8_t instance, uint8_t& serial_instance)
{
    ...
    case RangeFinder_TYPE_VL53L0X:
    	if (params[instance].address) {
			if (!_add_backend(AP_RangeFinder_VL53L0X::detect(state[instance], params[instance],
															 hal.i2c_mgr->get_device(1, /*((AP_RangeFinder_VL53L0X*)drivers[instance])->address*/0x29)))) {
				_add_backend(AP_RangeFinder_VL53L0X::detect(state[instance], params[instance],
															hal.i2c_mgr->get_device(0, /*((AP_RangeFinder_VL53L0X*)drivers[instance])->address*/0x29)));
			}
    	}
        break;
    ...
    case RangeFinder_TYPE_PX4_PWM:
        if (AP_RangeFinder_PX4_PWM::detect()) {
            drivers[instance] = new AP_RangeFinder_PX4_PWM(state[instance], params[instance], _powersave_range, estimated_terrain_height);
        }
        break;
#endif
